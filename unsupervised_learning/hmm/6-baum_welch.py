...
def baum_welch(Observations, Transition, Emission, Initial, iterations=1000):
    ...
    for _ in range(iterations):
        ...
        for t in range(1, T):
            F[:, t] = np.dot(F[:, t - 1], Transition) * Emission[:, Observations[t]]

        B = np.zeros((M, T))
        B[:, T - 1] = 1
        for t in range(T - 2, -1, -1):
            B[:, t] = np.dot(Transition, Emission[:, Observations[t + 1]] * B[:, t + 1])

        xi = np.zeros((M, M, T - 1))
        for t in range(T - 1):
            denom = np.dot(
                np.dot(F[:, t], Transition) * Emission[:, Observations[t + 1]],
                B[:, t + 1],
            )
            if denom == 0:
                continue
        ...
        gamma = np.hstack((gamma, np.sum(xi[:, :, T - 2], axis=0).reshape(M, 1)))
        for k in range(N):
